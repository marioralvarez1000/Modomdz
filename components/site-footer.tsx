...
import { getBorderStatus } from "@/lib/border-status";

export async function SiteFooter() {
  const border = await getBorderStatus();
  return (
    <footer className="site-footer">
      <div className="shell footer-grid">
        ...
        <div><b>Información útil</b><a href="https://www.argentina.gob.ar/smn" target="_blank" rel="noreferrer">Clima oficial</a><a href="https://www.argentina.gob.ar/seguridad/pasosinternacionales/detalle/ruta/29/Sistema-Cristo-Redentor" target="_blank" rel="noreferrer" title={border?`Según ${border.sourceUrl}, actualizado ${border.updatedAt}. Confirmá en la fuente oficial antes de salir.`:undefined}>Paso a Chile{border&&<span className={`border-status ${border.isOpen?"is-open":"is-closed"}`}>{border.label}</span>}</a><a href="/guias/emergencias-y-policia-turistica">Emergencias</a></div>
      </div>
