...
import { ArrowRight, Building2, Coffee, Grape, Map, Search, Sparkles, UtensilsCrossed } from "lucide-react";
...
export function PortalExplorer(){
  ...
  const {loading:aiLoading,slugs:aiSlugs}=useSmartSearch(query,results.length>0);
  const aiResults=useMemo(()=>aiSlugs.map(slug=>places.find(item=>item.slug===slug)).filter((item):item is typeof places[number]=>Boolean(item)),[aiSlugs]);
  return <>
    ...
    {query.trim()?<section className="shell explore-section"><div className="section-heading"><div><span className="overline">Resultados</span><h2>Lugares para vos</h2></div><span className="result-count">{results.length} resultados</span></div>{results.length?<div className="card-grid search-results">{results.map((item,index)=><ContentCard key={item.slug} item={item} priority={index<2}/>)}</div>:aiLoading?<div className="empty-state"><Sparkles/><h3>Buscando algo que combine con tu pedido…</h3></div>:aiResults.length?<><p className="ai-search-hint"><Sparkles size={15}/> Esto podría servirte</p><div className="card-grid search-results">{aiResults.map((item,index)=><ContentCard key={item.slug} item={item} priority={index<2}/>)}</div></>:<div className="empty-state"><Search/><h3>No encontramos ese lugar</h3><p>Probá con “bodega”, “restaurante”, “centro” o “montaña”.</p><button onClick={()=>setQuery("")}>Ver todos los lugares</button></div>}</section>:<>
